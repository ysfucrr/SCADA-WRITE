    ...
    switch (type) {
        ...
        case 'WRITE_REGISTER':
            if (payload.analyzerId && payload.address !== undefined && payload.value !== undefined) {
                ...
                backendLogger.warning("WRITE_REGISTER command received without necessary payload.", "Worker");
                parentPort?.postMessage({
                    type: 'WRITE_COMPLETED',
                    payload: {
                        requestId: payload.requestId,
        ...
        case 'WRITE_MULTIPLE_REGISTERS':
            if (payload.analyzerId && payload.address !== undefined && Array.isArray(payload.values)) {
                ...
                backendLogger.warning("WRITE_MULTIPLE_REGISTERS command received without necessary payload.", "Worker");
                parentPort?.postMessage({
                    type: 'WRITE_MULTIPLE_COMPLETED',
                    payload: {
                        requestId: payload.requestId,
