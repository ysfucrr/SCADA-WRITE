...
// PUT /api/units/[id]/floors/[floorId]/icon - Kat ikonu güncelle
export async function PUT(
  ...
  try {
    ...
    if (floor?.icon) {
      try {
        const filePath = path.join(process.cwd(), "public", floor.icon);
    ...
    return NextResponse.json({
      success: true,
      message: "Floor icon updated successfully",
    });
  } catch (error) {
    console.error("Error updating floor icon:", error);
    return NextResponse.json(
      { success: false, message: "Failed to update floor icon" },
      { status: 500 }
    );
  }
}
